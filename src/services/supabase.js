...
export const dbService = {
  ...
  async createChatSession(userId, documentId) {
    try {
      const { data, error } = await supabase
        .from('chat_sessions')
        .insert({
  ...
  async getChatSession(userId, documentId) {
    try {
      const { data, error } = await supabase
        .from('chat_sessions')
        .select('*')
  ...
  async getChatMessagesByDocument(userId, documentId, limit = 50) {
    try {
      const { data, error } = await supabase
        .from('chat_messages')
        .select('*')
      ...
      console.error('Error fetching chat messages by document:', error)
      return { success: false, error: error.message }
    }
  },
