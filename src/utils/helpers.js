...
export function getCurrentUserId() {
  try {
    ...
    console.warn('Unable to get current user ID:', error)
    return null
  }
}

/**
 * Get current user context for webhook tracking
 * Returns user ID and email if authenticated
 * @returns {Object} - User context object with id and email
 */
export function getUserContext() {
  try {
    ...
    return {
      ...
      user_email: authStore.userEmail || null,
      is_authenticated: authStore.isAuthenticated || false
    }
  } catch (error) {
    console.warn('Unable to get user context:', error)
    return {
      user_id: null,
      user_email: null,
